...
{
    pp2_tlv_t *tlv = (pp2_tlv_t *)malloc(sizeof_pp2_tlv_t + length);
    if (!tlv)
    {
        return NULL;
    }
...
static uint8_t tlv_array_append_tlv(tlv_array_t *tlv_array, pp2_tlv_t *tlv)
{
    if (!tlv_array->tlvs)
    {
        tlv_array->len = 0;
        tlv_array->size = 10;
        tlv_array->tlvs = (pp2_tlv_t**)malloc(tlv_array->size * sizeof(pp2_tlv_t*));
        if (!tlv_array->tlvs)
        {
            return 0;
        }
    ...
    {
        tlv_array->size += 5;
        pp2_tlv_t **tlvs = (pp2_tlv_t **)realloc(tlv_array->tlvs, tlv_array->size * sizeof(pp2_tlv_t*));
        if (!tlvs)
        {
            return 0;
        }
...
uint8_t pp_info_add_aws_vpce_id(pp_info_t *pp_info, const char *vpce_id)
{
    ...
    pp2_tlv_aws_t *pp2_tlv_aws = (pp2_tlv_aws_t *)malloc(length);
...
uint8_t pp_info_add_azure_linkid(pp_info_t *pp_info, uint32_t linkid)
{
    ...
    pp2_tlv_azure_t *pp2_tlv_azure = (pp2_tlv_azure_t *)malloc(length);
...
uint8_t *pp2_create_hdr(const pp_info_t *pp_info, uint16_t *pp2_hdr_len, int32_t *error)
{
    ...
    ::memcpy(proxy_hdr_v2.sig, PP2_SIG, sizeof(PP2_SIG));
    ...
    *pp2_hdr_len = sizeof(proxy_hdr_v2_t) + len;
    ...
    uint8_t *pp2_hdr = (uint8_t *)malloc(*pp2_hdr_len);
    if (!pp2_hdr)
    {
...
static uint8_t *pp1_create_hdr(const pp_info_t *pp_info, uint16_t *pp1_hdr_len, int32_t *error)
{
    ...
    /* Create the PROXY protocol header */
    uint8_t *pp1_hdr = (uint8_t *)malloc(*pp1_hdr_len);
    if (!pp1_hdr)
    {
        *error = ERR_HEAP_ALLOC;
        return NULL;
